<template>
  <div class="inpfield">
    <h1>Editing the product card</h1>
    <div>
...
export default {
  ...
  methods: {
    productCardRecord() {
      this.thisProd = this.prodData.find((el) => el.id === this.globalId);
      for (let key in this.thisProd) {
        this.vals[key] = this.thisProd[key];
      }
    },
    ...
    validTitle(event) {
      ...
      if (req.reqTest(val, req.valTitle)) {
        ...
        this.vals.nameprod = this.thisProd.nameprod;
        alert(
